...
class Service
{
    ...
    public function createClassListExcel(array $TableContent, array $tblPersonList)
    {
        ...
        $export->setValue($export->getCell($column++, $row), "Geb.");
        $export->setValue($export->getCell($column, $row++), "Geb. Ort");
        // Table Head
        ...
        // Table Border
        $export->setStyle($export->getCell(0, 1), $export->getCell(16, $row))->setAlignmentMiddle()->setBorderAll();
        // Column Width
        $column = 0;
        ...
        $export->setStyle($export->getCell($column, 0), $export->getCell($column, $row++))->setColumnWidth(20)->setFontSize(9);
        $row++;
