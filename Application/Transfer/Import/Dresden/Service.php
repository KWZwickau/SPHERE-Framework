...
//                            $error[] = 'Zeile: ' . ($RunY + 1) . ' Keine Zuordnung ohne Nachname.';
                            ...
                            if ($tblPerson === false) {
                                $error[] = 'Zeile: ' . ($RunY + 1) . ' Die Person '.new Bold($firstName.' '.$lastName).' wurde nicht gefunden.';
                            } else {
                                $countAllocation++;
...
//                    Debugger::screenDump($error);
                    ...
                    return
                        new Success('Es wurden ' . $countAllocation . ' Personen-Gruppen zuweisungen erfolgreich angelegt.')
                        . new Layout(new LayoutGroup(new LayoutRow(new LayoutColumn(
                            new Panel(
                                'Fehler',
                                $error,
                    ...
                    for ($RunY = 1; $RunY < $Y; $RunY++) {
                        ...
                        if ($firstName === '' || $lastName === '') {
                            ...
                            if ($tblPerson === false) {
                                ...
                                $day = trim($Document->getValue($Document->getCell($Location['GebDatum'],
                                    $RunY)));
                                if ($day !== '') {
                                    try {
