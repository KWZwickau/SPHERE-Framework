    ...
    public function buildPages(TblPerson $tblPerson = null)
    {
        ...
        return (new Page())
            ...
            ->addSlice((new Slice())
                ->addSection((new Section())
                    ->addElementColumn((new Element())
                        ->setContent('Durchschnitt der Noten ausschließlich Englisch')
                        ->styleMarginTop('20px')
                        , '80%')
                    ->addElementColumn((new Element())
                        ->setContent('{% if(Content.P' . $personId . '.Grade.Data.AverageWithoutEN is not empty) %}
                                    {{ Content.P' . $personId . '.Grade.Data.AverageWithoutEN }}
                                {% else %}
                                    ---
                                {% endif %}')
                        ->stylePaddingTop()
