...
class AccountingDownload implements IModuleInterface
{
     ...
     * @return bool|string
     */
    public function downloadAccountingList($GroupId = null)
    {

        if(($tblGroup = Group::useService()->getGroupById($GroupId))){
            if(($ExcelContent = $this->getAccountingContent($tblGroup))){
                $fileLocation = $this->createAccountingDownload($ExcelContent);

                return FileSystem::getDownload($fileLocation->getRealPath(),
                    "Fakturierung ".$tblGroup->getName()
                    ." ".date("d-m-Y").".xlsx")->__toString();
            }
        }
        return false;
    }
     ...
     * @return array|false
     */
    private function getAccountingContent(TblGroup $tblGroup)
    {
...
//        $tblPersonList = (new Sorter($tblPersonList))->sortObjectBy('getLastFirstName');
     ...
     */
    private function createAccountingDownload($ExcelContent = array())
    {
