...
1. /sign-in (without valid processToken)
   - Invalidate all factors (except credentials)
   - Create ProcessToken A
   - 302 > /credentials (or "next" mfa if factor is solved)
       - processToken A
2. /credentials + processToken A
    - Error: 4xx/5xx
    - Success: 302 > /sign-in
        - processToken B
3. /sign-in + processToken B
    - 201: Token
    - 302 > /yubikey
        - processToken C
4. /yubikey + processToken C
    - Error: 4xx/5xx
    - Success: 302 > /sign-in
        - processToken D
5. /sign-in + processToken D
   - 201: Token
   - Remove processToken
...
- {Public Routes}
    - Defined in App/Dispatcher
    - Example
        - /app/authentication/process/sign-{in|out}
        - /app/authentication/factor/{credentials|yubikey...}
