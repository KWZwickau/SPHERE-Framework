(function( $ )
{
    ...
    Selectpicker.prototype = {
            ...
            if (this.multiple && this.options.selectedTextFormat.indexOf( 'count' ) > -1) {
                ...
                if ((max.length > 1 && selectedItems.length > max[1]) || (max.length == 1 && selectedItems.length >= 2)) {
                    ...
                    var totalCount = this.$element.find( 'option' ).not( '[data-divider="true"], [data-hidden="true"]' + notDisabled ).length,
                        tr8nText = (typeof this.options.countSelectedText === 'function') ? this.options.countSelectedText( selectedItems.length,
                            totalCount ) : this.options.countSelectedText;
        ...
        liHeight: function()
        {
            ...
            var $selectClone = this.$menu.parent().clone().children( '.dropdown-toggle' ).prop( 'autofocus',
        ...
        setSize: function()
        {
            ...
            if (this.options.size == 'auto') {
                ...
                if (this.options.size && this.options.size != 'auto' && menu.find( 'li' + notDisabled ).length > this.options.size) {
                    var optIndex = this.$lis.not( '.divider' + notDisabled ).children().slice( 0,
        ...
        selectPosition: function()
        {
            var that = this,
                ...
                {
                    $drop.addClass( $element.attr( 'class' ).replace( /form-control/gi, '' ) ).toggleClass( 'dropup',
        ...
        setDisabled: function( index, disabled )
        {
            this.findLis();
            if (disabled) {
                this.$lis.filter( '[data-original-index="' + index + '"]' ).addClass( 'disabled' ).find( 'a' ).attr( 'href',
                    '#' ).attr( 'tabindex', -1 );
            } else {
                this.$lis.filter( '[data-original-index="' + index + '"]' ).removeClass( 'disabled' ).find( 'a' ).removeAttr( 'href' ).attr( 'tabindex',
                    0 );
            }
        },
                                ...
                                if (maxOptions && maxOptions == 1) {
                                    ...
                                    if (maxOptionsGrp && maxOptionsGrp == 1) {
                                        ...
                                        var maxOptionsArr = (typeof that.options.maxOptionsText === 'function') ?
                                                that.options.maxOptionsText( maxOptions,
                                                    maxOptionsGrp ) : that.options.maxOptionsText,
        ...
        {
            this.findLis();
            this.$element.find( 'option:enabled' ).not( '[data-divider]' ).not( '[data-hidden]' ).prop( 'selected',
                true );
    ...
    function Plugin( option, event )
    {
        ...
        var chain = this.each( function()
        {
            ...
            if ($this.is( 'select' )) {
                ...
                if (!data) {
                    var config = $.extend( {}, Selectpicker.DEFAULTS, $.fn.selectpicker.defaults || {}, $this.data(),
                        options );
    ...
    $( document )
        .data( 'keycount', 0 )
        .on( 'keydown', '.bootstrap-select [data-toggle=dropdown], .bootstrap-select [role=menu], .bs-searchbox input',
