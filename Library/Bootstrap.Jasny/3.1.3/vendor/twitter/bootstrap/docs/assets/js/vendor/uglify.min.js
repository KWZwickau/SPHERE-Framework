(function( exports, global )
{
    ...
    var AST_ObjectKeyVal = DEFNODE( "ObjectKeyVal", "quote",
        {$documentation: "A key: value object property", $propdoc: {quote: "[string] the original quote character"}},
        AST_ObjectProperty );
    ...
    AST_Toplevel.DEFMETHOD( "_default_mangler_options", function( options )
    {
        return defaults( options,
            {except: [], eval: false, sort: false, toplevel: false, screw_ie8: false, keep_fnames: false} )
    } );
    ...
    (function()
    {
                    ...
                    if (body && body.args) {
                        body.args.forEach( function( argument, index, array )
                        {
                            ...
                            if (argument instanceof AST_Lambda && comments.length && has_inject( comments[0] )) {
                                array[index] = make_array( argument,
            ...
            if (compressor.option( "booleans" ) && compressor.in_boolean_context())switch (self.operator) {
                    ...
                    if (ll.length > 1 && !ll[1] || rr.length > 1 && !rr[1]) {
                        ...
                        if (self.left.has_side_effects( compressor )) {
                            return make_node( AST_Seq, self,
                                {car: self.left, cdr: make_node( AST_False )} ).optimize( compressor )
                        }
                ...
                case"||":
                    ...
                    if (ll.length > 1 && ll[1] || rr.length > 1 && rr[1]) {
                        ...
                        if (self.left.has_side_effects( compressor )) {
                            return make_node( AST_Seq, self,
                                {car: self.left, cdr: make_node( AST_True )} ).optimize( compressor )
                        }
                        return make_node( AST_True, self )
                    }
                    if (ll.length > 1 && !ll[1]) {
        ...
        OPT( AST_Conditional, function( self, compressor )
        {
            ...
            if (consequent instanceof AST_True && alternative instanceof AST_False) {
                ...
                return make_node( AST_UnaryPrefix, self.condition, {operator: "!", expression: self.condition} )
            }
    ...
    {
        options = defaults( options, {reserved: null, cache: null, only_cache: false} );
