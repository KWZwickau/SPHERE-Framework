...
class VisitorProfile
{
    ...
    public function makeVisitorProfile(DataTable $visits, $visitorId, $segment)
    {
        ...
        $this->profile['totalVisitDurationPretty'] = $formatter->getPrettyTimeFromSeconds($this->profile['totalVisitDuration'],
            true);
    ...
    private function handleIfEcommerceAction($action)
    {
        ...
        if ($action['type'] == Piwik::LABEL_ID_GOAL_IS_ECOMMERCE_ORDER) {
            ...
            if ($action['type'] == Piwik::LABEL_ID_GOAL_IS_ECOMMERCE_CART) {
    ...
    private function handleAdjacentVisitorIds(DataTable $visits, $visitorId, $segment)
    {
        ...
        $model = new Model();
        $this->profile['nextVisitorId'] = $model->queryAdjacentVisitorId($this->idSite, $visitorId, $latestVisitTime,
            $segment, $getNext = true);
