    ...
    $.extend(RealtimeMap.prototype, UIControl.prototype, {
        ...
        _initStandaloneMap: function ()
        {
            ...
            $('#secondNavBar').on('piwikSwitchPage', function (event, item)
            {
                ...
                var clickedMenuIsNotMap = !href || (href.indexOf(
                        'module=UserCountryMap&action=realtimeWorldMap') == -1);
        ...
        run: function ()
        {
            ...
            var self = this,
                ...
                colors = colorManager.getColors('realtime-map', [
                    'white-bg', 'white-fill', 'black-bg', 'black-fill', 'visit-stroke',
            ...
            function _reportParams(firstRun)
            {
                return $.extend(config.reqParams, {
                    ...
                    showColumns: [
                        ...
                        'actions', 'searches', 'goalConversions', 'visitorId', 'userId'
                    ].join(','),
            ...
            function relativeTime(ds)
            {
                ...
                {
                    return '<strong>' + Math.round(val) + '</strong>';
                };
            ...
            function visitTooltip(r)
            {
                ...
                return '<h3>' + (r.city ? r.city + ' / ' : '') + r.country + '</h3>' +
                    ...
                    (ad && ad.length && ad[ad.length - 1].pageTitle ? '<em>' + ad[ad.length - 1].pageTitle + '</em><br/>' : '') +
                        // time of visit
                    '<div class="rel-time" data-actiontime="' + r.lastActionTimestamp + '">' + relativeTime(
                        ds) + '</div>' +
                    ...
                    '<small>' + _.local_time + ': ' + r.visitLocalTime + '</small><br />' +
                        // goals, if available
                    (self.config.siteHasGoals && r.goalConversions ? '<small>' + _.goal_conversions.replace('%s',
                        '<strong>' + r.goalConversions + '</strong>') +
                    (r.searches > 0 ? ', ' + _.searches.replace('%s', r.searches) : '') + '</small><br />' : '') +
                        // actions and searches
                    '<small>' + _.actions.replace('%s', '<strong>' + r.actions + '</strong>') +
                    (r.searches > 0 ? ', ' + _.searches.replace('%s',
                        '<strong>' + r.searches + '</strong>') : '') + '</small>';
            }
                ...
                function gotNewReport(report)
                {
                    ...
                    if (!report.length) {
                        ...
                        if (removeOldVisits) {
                            ...
                            {
                                if (r.lastActionTimestamp < oldest) {
                                    _removed++;
                                }
                        ...
                        if (dur < 60) {
                            ...
                            if (dur < 3600) {
                                d = Math.ceil(dur / 60) + ' ' + _.minutes;
                            } else {
                                d = Math.ceil(dur / 3600) + ' ' + _.hours;
                            }
                        }
            ...
            updateMap(location.hash && (location.hash == '#world' || location.hash.match(
                /^#[A-Z][A-Z]$/)) ? location.hash.substr(1) : 'world'); // TODO: restore last state
