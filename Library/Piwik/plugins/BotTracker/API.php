...
class API extends \Piwik\Plugin\API
{
    ...
    {
        $rows = Db::get()->fetchAll("SELECT `idsite`, `botId`, `botName`, `botActive`, `botKeyword`, `extra_stats` FROM " . Common::prefixTable('bot_db') . " WHERE `idsite` = ? ORDER BY `botId`",
            array($idsite));
    ...
    {
        $rows = Db::get()->fetchAll("SELECT * FROM " . Common::prefixTable('bot_db') . " WHERE `botActive` = 1 AND idSite= ? ORDER BY `botId`",
            array($idSite));
    ...
    static function convertBotLastVisitToLocalTime($rows, $idSite)
    {
        ...
        foreach ($rows as &$row) {
            if ($row['botLastVisit'] == '0000-00-00 00:00:00') {
                ...
                if ($row['botLastVisit'] == '2000-01-01 00:00:00') {
                    $row['botLastVisit'] = " - ";
                } else {
    ...
    {
        $rows = Db::get()->fetchAll("SELECT `botName`, `botCount` FROM " . Common::prefixTable('bot_db') . " WHERE `idSite`= ? ORDER BY `botCount` DESC LIMIT 10",
            array($idSite));
    ...
    {
        $rows = Db::get()->fetchAll("SELECT * FROM " . Common::prefixTable('bot_db') . " WHERE `botName` = ? AND `idSite`= ? ORDER BY `botId`",
            array($botName, $idSite));
        $rows = self::convertBotLastVisitToLocalTime($rows, $idSite);
        return $rows;
    }
    ...
    {
        $rows = Db::get()->fetchAll("SELECT * FROM " . Common::prefixTable('bot_db') . " WHERE idSite= ? ORDER BY `botId`",
            array($idSite));
        $rows = self::convertBotLastVisitToLocalTime($rows, $idSite);
        // convert this array to a DataTable object
        return DataTable::makeFromIndexedArray($rows);
