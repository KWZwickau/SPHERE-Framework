...
class VisitorGeolocator
{
    ...
    {
        $visitFieldsToSelect = array_merge(array('idvisit', 'location_ip'),
            array_keys(VisitorGeolocator::$logVisitFieldsToUpdate));
        ...
        $self = $this;
        $this->dao->forAllLogs('log_visit', $visitFieldsToSelect, $conditions, $iterationStep,
            function ($logs) use ($self, $onLogProcessed) {
                foreach ($logs as $row) {
    ...
    public function attributeExistingVisit($visit, $useClassCache = true)
    {
        ...
        if (empty($visit['location_ip'])) {
            $this->logger->debug('Empty location_ip field for idvisit = %s. Skipping re-attribution.',
                array('idvisit' => $idVisit));
            return null;
        }
        ...
        if (!empty($valuesToUpdate)) {
            ...
            $this->logger->debug('Nothing to update for idvisit = %s (IP = {ip}). Existing location info is same as geolocated.',
                array(
                    'idVisit' => $idVisit,
                    'ip' => $ip
                ));
    ...
    private function getLocationObject(LocationProvider $provider, $userInfo)
    {
        ...
        Common::printDebug("GEO: Found IP $ipAddress location (provider '" . $providerId . "'): " . var_export($location,
                true));
