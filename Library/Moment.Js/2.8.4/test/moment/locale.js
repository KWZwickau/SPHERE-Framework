...
exports.locale = {
    ...
    {
        test.equal( moment.locale( ['non-existent', 'fr', 'also-non-existent'] ), 'fr',
            'passing an array uses the first valid locale' );
        test.equal( moment.locale( ['es', 'fr', 'also-non-existent'] ), 'es',
            'passing an array uses the first valid locale' );
        test.done();
    },
    ...
    'library ensure inheritance LT L LL LLL LLLL': function( test )
    {
        ...
        test.equal( moment().locale( locale ).calendar(), 'sameDay -LT-',
            'Should use instance locale in LT formatting' );
        test.equal( moment().add( 1, 'days' ).locale( locale ).calendar(), 'nextDay -L-',
            'Should use instance locale in L formatting' );
        test.equal( moment().add( -1, 'days' ).locale( locale ).calendar(), 'lastDay -LLL-',
            'Should use instance locale in LL formatting' );
        test.equal( moment().add( 4, 'days' ).locale( locale ).calendar(), 'nextWeek -LL-',
            'Should use instance locale in LLL formatting' );
        test.equal( moment().add( -4, 'days' ).locale( locale ).calendar(), 'lastWeek -LLLL-',
    ...
    'library localeData': function( test )
    {
        ...
        test.equal( moment.localeData( moment().locale( 'es' ) ).months( jan ), 'enero',
    ...
    'instance locale method': function( test )
    {
        ...
        test.equal( moment( [2012, 5, 6] ).locale( 'es' ).format( 'MMMM' ), 'junio',
            'Use the instance specific locale' );
        test.equal( moment( [2012, 5, 6] ).format( 'MMMM' ), 'June',
    ...
    'duration locale method': function( test )
    {
        ...
        test.equal( moment.duration( {seconds: 44} ).locale( 'es' ).humanize(), 'unos segundos',
            'Use the instance specific locale' );
    ...
    {
        test.equal( moment.duration().lang(), moment.duration().localeData(),
    ...
    'from relative time future': function( test )
    {
        ...
        test.equal( start.from( moment( [2007, 1, 28] ).subtract( {s: 44} ) ), 'in a few seconds',
        ...
        test.equal( start.from( moment( [2007, 1, 28] ).subtract( {s: 90} ) ), 'in 2 minutes',
            '90 seconds = 2 minutes' );
        test.equal( start.from( moment( [2007, 1, 28] ).subtract( {m: 44} ) ), 'in 44 minutes',
        ...
        test.equal( start.from( moment( [2007, 1, 28] ).subtract( {d: 315} ) ), 'in 10 months',
            '315 days = 10 months' );
    ...
    'from relative time past': function( test )
    {
        ...
        test.equal( start.from( moment( [2007, 1, 28] ).add( {s: 44} ) ), 'a few seconds ago',
            '44 seconds = a few seconds' );
    ...
    'month name callback function': function( test )
    {
        ...
        test.equal( moment().format( '[test] dd ddd dddd MMM MMMM' ), 'test test test test test test',
            'format month name function should be able to access the format string' );
        test.equal( moment( [2011, 0, 1] ).format( 'dd ddd dddd MMM MMMM' ), 'date date date date date',
            'format month name function should be able to access the moment object' );
        test.equal( moment( [2011, 0, 2] ).format( 'dd ddd dddd MMM MMMM' ), 'default default default default default',
            'format month name function should be able to access the moment object' );
    ...
    'changing parts of a locale config': function( test )
    {
        ...
        test.equal( moment( [2011, 0, 1] ).format( 'MMMM' ), 'a',
        ...
        test.equal( moment( [2011, 0, 1] ).format( 'MMMM MMM' ), 'a A',
    ...
    'start/endOf week feature for first-day-is-monday locales': function( test )
    {
        ...
        moment.locale( 'monday-lang' );
        test.equal( moment( [2013, 0, 1] ).startOf( 'week' ).day(), 1,
            'for locale monday-lang first day of the week should be monday' );
        test.equal( moment( [2013, 0, 1] ).endOf( 'week' ).day(), 0,
