...
exports.format = {
    ...
    'format escape brackets': function( test )
    {
        ...
        test.equal( b.format( '[L LL LLL LLLL aLa]' ), 'L LL LLL LLLL aLa',
            'localized tokens with escaped localized tokens' );
        test.equal( b.format( '[LLL] LLL' ), 'LLL February 14, 2009 3:25 PM',
    ...
    'format timezone': function( test )
    {
        ...
        test.ok( b.format( 'Z' ).match( /^[\+\-]\d\d:\d\d$/ ),
    ...
    'format multiple with zone': function( test )
    {
        ...
        test.equals( b.format( 'YYYY-MM' ), '2012-10',
    ...
    'zone': function( test )
    {
        ...
        if (moment().zone() > 0) {
            test.ok( moment().format( 'ZZ' ).indexOf( '-' ) > -1,
        ...
        if (moment().zone() === 0) {
            ...
            test.equal( moment().zone() % 15, 0,
                'moment.fn.zone should be a multiple of 15 (was ' + moment().zone() + ')' );
        }
    ...
    'iso week year formats': function( test )
    {
        ...
        for (i in cases) {
            ...
            test.equal( isoWeekYear.slice( 2, 4 ), formatted2,
    ...
    'calendar day timezone': function( test )
    {
        ...
        test.equal( moment( b ).utc().calendar(), 'Yesterday at 11:59 PM',
            'Yesterday at 11:59 PM, not Today, or the wrong time' );
        test.equal( moment( c ).local().calendar(), 'Yesterday at 11:59 PM',
            'Yesterday at 11:59 PM, not Today, or the wrong time' );
        test.equal( moment( c ).local().calendar( d ), 'Tomorrow at 11:59 PM',
