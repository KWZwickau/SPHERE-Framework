...
exports['locale:lv'] = {
    ...
    'from': function( test )
    {
        ...
        test.equal( start.from( moment( [2007, 1, 28] ).add( {s: 44} ), true ), 'dažas sekundes',
            '44 seconds = seconds' );
        ...
        test.equal( start.from( moment( [2007, 1, 28] ).add( {m: 44} ), true ), '44 minūtes',
    ...
    'calendar last week': function( test )
    {
        ...
        for (i = 2; i < 7; i++) {
            ...
            test.equal( m.calendar(), m.format( '[Pagājušā] dddd [pulksten] LT' ),
