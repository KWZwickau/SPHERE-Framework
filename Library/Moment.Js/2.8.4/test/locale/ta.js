...
exports['locale:ta'] = {
    ...
    'from': function( test )
    {
        ...
        test.equal( start.from( moment( [2007, 1, 28] ).add( {s: 44} ), true ), 'ஒரு சில விநாடிகள்',
            '44 விநாடிகள் = ஒரு சில விநாடிகள்' );
        test.equal( start.from( moment( [2007, 1, 28] ).add( {s: 45} ), true ), 'ஒரு நிமிடம்',
            '45 விநாடிகள் = ஒரு நிமிடம்' );
        test.equal( start.from( moment( [2007, 1, 28] ).add( {s: 89} ), true ), 'ஒரு நிமிடம்',
            '89 விநாடிகள் = ஒரு நிமிடம்' );
        test.equal( start.from( moment( [2007, 1, 28] ).add( {s: 90} ), true ), '2 நிமிடங்கள்',
            '90 விநாடிகள் = 2 நிமிடங்கள்' );
        test.equal( start.from( moment( [2007, 1, 28] ).add( {m: 44} ), true ), '44 நிமிடங்கள்',
            '44 நிமிடங்கள் = 44 நிமிடங்கள்' );
        test.equal( start.from( moment( [2007, 1, 28] ).add( {m: 45} ), true ), 'ஒரு மணி நேரம்',
            '45 நிமிடங்கள் = ஒரு மணி நேரம்' );
        test.equal( start.from( moment( [2007, 1, 28] ).add( {m: 89} ), true ), 'ஒரு மணி நேரம்',
            '89 நிமிடங்கள் = ஒரு மணி நேரம்' );
        test.equal( start.from( moment( [2007, 1, 28] ).add( {m: 90} ), true ), '2 மணி நேரம்',
            '90 நிமிடங்கள் = 2 மணி நேரம்' );
        test.equal( start.from( moment( [2007, 1, 28] ).add( {h: 5} ), true ), '5 மணி நேரம்',
            '5 மணி நேரம் = 5 மணி நேரம்' );
        test.equal( start.from( moment( [2007, 1, 28] ).add( {h: 21} ), true ), '21 மணி நேரம்',
        ...
        test.equal( start.from( moment( [2007, 1, 28] ).add( {d: 25} ), true ), '25 நாட்கள்',
        ...
        test.equal( start.from( moment( [2007, 1, 28] ).add( {d: 46} ), true ), '2 மாதங்கள்',
            '46 நாட்கள் = 2 மாதங்கள்' );
        test.equal( start.from( moment( [2007, 1, 28] ).add( {d: 74} ), true ), '2 மாதங்கள்',
            '75 நாட்கள் = 2 மாதங்கள்' );
        test.equal( start.from( moment( [2007, 1, 28] ).add( {d: 76} ), true ), '3 மாதங்கள்',
            '76 நாட்கள் = 3 மாதங்கள்' );
        test.equal( start.from( moment( [2007, 1, 28] ).add( {M: 1} ), true ), 'ஒரு மாதம்', '1 மாதம் = ஒரு மாதம்' );
        test.equal( start.from( moment( [2007, 1, 28] ).add( {M: 5} ), true ), '5 மாதங்கள்',
            '5 மாதங்கள் = 5 மாதங்கள்' );
        test.equal( start.from( moment( [2007, 1, 28] ).add( {d: 345} ), true ), 'ஒரு வருடம்',
            '345 நாட்கள் = ஒரு வருடம்' );
        test.equal( start.from( moment( [2007, 1, 28] ).add( {d: 548} ), true ), '2 ஆண்டுகள்',
            '548 நாட்கள் = 2 ஆண்டுகள்' );
        test.equal( start.from( moment( [2007, 1, 28] ).add( {y: 1} ), true ), 'ஒரு வருடம்', '1 வருடம் = ஒரு வருடம்' );
        test.equal( start.from( moment( [2007, 1, 28] ).add( {y: 5} ), true ), '5 ஆண்டுகள்',
