...
exports['locale:be'] = {
    ...
    'format month case with escaped symbols': function( test )
    {
        ...
        for (i = 0; i < 12; i++) {
            ...
            test.equal( moment( [2013, i, 1] ).format( 'D[-ы дзень] MMMM' ), '1-ы дзень ' + months.accusative[i],
    ...
    'from': function( test )
    {
        ...
        test.equal( start.from( moment( [2007, 1, 28] ).add( {s: 44} ), true ), 'некалькі секунд',
        ...
        test.equal( start.from( moment( [2007, 1, 28] ).add( {m: 31} ), true ), '31 хвіліна',
            '31 minutes = 31 minutes' );
        test.equal( start.from( moment( [2007, 1, 28] ).add( {m: 44} ), true ), '44 хвіліны',
