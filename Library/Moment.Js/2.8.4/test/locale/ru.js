...
exports['locale:ru'] = {
    ...
    'format month case': function( test )
    {
        ...
        for (i = 0; i < 12; i++) {
            test.equal( moment( [2011, i, 1] ).format( 'D MMMM' ), '1 ' + months.accusative[i],
    ...
    'format month short case': function( test )
    {
        ...
        for (i = 0; i < 12; i++) {
            test.equal( moment( [2011, i, 1] ).format( 'D MMM' ), '1 ' + monthsShort.accusative[i],
                '1 ' + monthsShort.accusative[i] );
            test.equal( moment( [2011, i, 1] ).format( 'MMM' ), monthsShort.nominative[i],
    ...
    'format month case with escaped symbols': function( test )
    {
        ...
        for (i = 0; i < 12; i++) {
            test.equal( moment( [2013, i, 1] ).format( 'D[] MMMM' ), '1 ' + months.accusative[i],
                '1 ' + months.accusative[i] );
            test.equal( moment( [2013, i, 1] ).format( '[<i>]D[</i>] [<b>]MMMM[</b>]' ),
                '<i>1</i> <b>' + months.accusative[i] + '</b>', '1 <b>' + months.accusative[i] + '</b>' );
            test.equal( moment( [2013, i, 1] ).format( 'D[-й день] MMMM' ), '1-й день ' + months.accusative[i],
                '1-й день ' + months.accusative[i] );
            test.equal( moment( [2013, i, 1] ).format( 'D, MMMM' ), '1, ' + months.nominative[i],
    ...
    'format month short case with escaped symbols': function( test )
    {
        ...
        for (i = 0; i < 12; i++) {
            test.equal( moment( [2013, i, 1] ).format( 'D[] MMM' ), '1 ' + monthsShort.accusative[i],
                '1 ' + monthsShort.accusative[i] );
            test.equal( moment( [2013, i, 1] ).format( '[<i>]D[</i>] [<b>]MMM[</b>]' ),
                '<i>1</i> <b>' + monthsShort.accusative[i] + '</b>', '1 <b>' + monthsShort.accusative[i] + '</b>' );
            test.equal( moment( [2013, i, 1] ).format( 'D[-й день] MMM' ), '1-й день ' + monthsShort.accusative[i],
                '1-й день ' + monthsShort.accusative[i] );
            test.equal( moment( [2013, i, 1] ).format( 'D, MMM' ), '1, ' + monthsShort.nominative[i],
                '1, ' + monthsShort.nominative[i] );
        }
        test.done();
    },
    ...
    'from': function( test )
    {
        ...
        test.equal( start.from( moment( [2007, 1, 28] ).add( {s: 44} ), true ), 'несколько секунд',
            '44 seconds = seconds' );
        ...
        test.equal( start.from( moment( [2007, 1, 28] ).add( {m: 31} ), true ), '31 минута',
            '31 minutes = 31 minutes' );
        test.equal( start.from( moment( [2007, 1, 28] ).add( {m: 44} ), true ), '44 минуты',
