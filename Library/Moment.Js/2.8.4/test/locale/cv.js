...
exports['locale:cv'] = {
    ...
    'from': function( test )
    {
        ...
        test.equal( start.from( moment( [2007, 1, 28] ).add( {s: 44} ), true ), 'пĕр-ик çеккунт',
    ...
    'calendar next week': function( test )
    {
        ...
        for (i = 2; i < 7; i++) {
            ...
            test.equal( m.calendar(), m.format( '[Çитес] dddd LT [сехетре]' ),
    ...
    'calendar last week': function( test )
    {
        ...
        for (i = 2; i < 7; i++) {
            ...
            m.hours( 0 ).minutes( 0 ).seconds( 0 ).milliseconds( 0 );
            test.equal( m.calendar(), m.format( '[Иртнĕ] dddd LT [сехетре]' ),
                'Today - ' + i + ' days beginning of day' );
            m.hours( 23 ).minutes( 59 ).seconds( 59 ).milliseconds( 999 );
