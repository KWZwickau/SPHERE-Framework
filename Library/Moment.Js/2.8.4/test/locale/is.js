...
exports['locale:is'] = {
    ...
    'from': function( test )
    {
        ...
        test.equal( start.from( moment( [2007, 1, 28] ).add( {s: 44} ), true ), 'nokkrar sekúndur',
        ...
        test.equal( start.from( moment( [2007, 1, 28] ).add( {m: 44} ), true ), '44 mínútur',
            '44 minutes = 44 minutes' );
        test.equal( start.from( moment( [2007, 1, 28] ).add( {m: 21} ), true ), '21 mínúta',
        ...
        test.equal( start.from( moment( [2007, 1, 28] ).add( {m: 90} ), true ), '2 klukkustundir',
            '90 minutes = 2 hours' );
        test.equal( start.from( moment( [2007, 1, 28] ).add( {h: 5} ), true ), '5 klukkustundir', '5 hours = 5 hours' );
        test.equal( start.from( moment( [2007, 1, 28] ).add( {h: 21} ), true ), '21 klukkustund',
            '21 hours = 21 hours' );
    ...
    'calendar last week': function( test )
    {
        ...
        for (i = 2; i < 7; i++) {
            ...
            m.hours( 0 ).minutes( 0 ).seconds( 0 ).milliseconds( 0 );
            test.equal( m.calendar(), m.format( '[síðasta] dddd [kl.] LT' ),
