...
exports['locale:az'] = {
    ...
    'format': function( test )
    {
        ...
        for (i = 0; i < DDDo.length; i++) {
            DDDoDt = moment( [2010] );
            test.equal( DDDoDt.add( DDDo[i][0], 'days' ).format( 'DDDo' ), DDDo[i][1],
                DDDo[i][0] + ' ---> ' + DDDo[i][1] );
        }
        test.done();
    },
    ...
    'from': function( test )
    {
        ...
        test.equal( start.from( moment( [2007, 1, 28] ).add( {s: 44} ), true ), 'birneçə saniyyə',
        ...
        test.equal( start.from( moment( [2007, 1, 28] ).add( {m: 44} ), true ), '44 dəqiqə',
    ...
    'calendar next week': function( test )
    {
        ...
        for (i = 2; i < 7; i++) {
            m = moment().add( {d: i} );
            test.equal( m.calendar(), m.format( '[gələn həftə] dddd [saat] LT' ),
    ...
    'calendar last week': function( test )
    {
        ...
        for (i = 2; i < 7; i++) {
            ...
            test.equal( m.calendar(), m.format( '[keçən həftə] dddd [saat] LT' ),
                'Today - ' + i + ' days current time' );
            m.hours( 0 ).minutes( 0 ).seconds( 0 ).milliseconds( 0 );
            test.equal( m.calendar(), m.format( '[keçən həftə] dddd [saat] LT' ),
                'Today - ' + i + ' days beginning of day' );
