...
exports['locale:uk'] = {
    ...
    'from': function( test )
    {
        ...
        test.equal( start.from( moment( [2007, 1, 28] ).add( {s: 44} ), true ), 'декілька секунд',
            '44 seconds = seconds' );
        ...
        test.equal( start.from( moment( [2007, 1, 28] ).add( {m: 44} ), true ), '44 хвилини',
    ...
    'calendar next week': function( test )
    {
        ...
        for (i = 2; i < 7; i++) {
            m = moment().add( {d: i} );
            test.equal( m.calendar(), m.format( '[У] dddd [о' + (m.hours() === 11 ? 'б' : '') + '] LT' ),
