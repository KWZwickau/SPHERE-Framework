...
exports['locale:de'] = {
    ...
    'from': function( test )
    {
        ...
        test.equal( start.from( moment( [2007, 1, 28] ).add( {s: 44} ), true ), 'ein paar Sekunden',
            '44 seconds = a few seconds' );
        test.equal( start.from( moment( [2007, 1, 28] ).add( {s: 45} ), true ), 'eine Minute',
            '45 seconds = a minute' );
        test.equal( start.from( moment( [2007, 1, 28] ).add( {s: 89} ), true ), 'eine Minute',
            '89 seconds = a minute' );
        test.equal( start.from( moment( [2007, 1, 28] ).add( {s: 90} ), true ), '2 Minuten', '90 seconds = 2 minutes' );
        test.equal( start.from( moment( [2007, 1, 28] ).add( {m: 44} ), true ), '44 Minuten',
    ...
    'calendar last week': function( test )
    {
        ...
        for (i = 2; i < 7; i++) {
            ...
            test.equal( m.calendar(), m.format( '[letzten] dddd [um] LT [Uhr]' ),
