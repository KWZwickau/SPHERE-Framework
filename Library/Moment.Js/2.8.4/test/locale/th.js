...
exports['locale:th'] = {
    ...
    'from': function( test )
    {
        ...
        test.equal( start.from( moment( [2007, 1, 28] ).add( {s: 44} ), true ), 'ไม่กี่วินาที',
    ...
    'calendar day': function( test )
    {
        ...
        test.equal( moment( a ).add( {d: 1} ).calendar(), 'พรุ่งนี้ เวลา 2 นาฬิกา 0 นาที',
            'tomorrow at the same time' );
        test.equal( moment( a ).subtract( {h: 1} ).calendar(), 'วันนี้ เวลา 1 นาฬิกา 0 นาที', 'Now minus 1 hour' );
        test.equal( moment( a ).subtract( {d: 1} ).calendar(), 'เมื่อวานนี้ เวลา 2 นาฬิกา 0 นาที',
    ...
    'calendar last week': function( test )
    {
        ...
        for (i = 2; i < 7; i++) {
            ...
            m.hours( 0 ).minutes( 0 ).seconds( 0 ).milliseconds( 0 );
            test.equal( m.calendar(), m.format( '[วัน]dddd[ที่แล้ว เวลา] LT' ),
