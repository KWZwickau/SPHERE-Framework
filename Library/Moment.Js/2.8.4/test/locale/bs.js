...
exports['locale:bs'] = {
    ...
    'from': function( test )
    {
        ...
        test.equal( start.from( moment( [2007, 1, 28] ).add( {s: 44} ), true ), 'par sekundi',
            '44 seconds = a few seconds' );
        test.equal( start.from( moment( [2007, 1, 28] ).add( {s: 45} ), true ), 'jedna minuta',
            '45 seconds = a minute' );
        test.equal( start.from( moment( [2007, 1, 28] ).add( {s: 89} ), true ), 'jedna minuta',
            '89 seconds = a minute' );
