...
class Unit_Crypt_RC2Test extends PhpseclibTestCase
{
    ...
    public function testEncryptPadding()
    {
        ...
        $this->assertEquals($result, $internal,
            'Failed asserting that the internal engine produced the correct result');

        $rc2->setPreferredEngine(Base::ENGINE_MCRYPT);
        if ($rc2->getEngine() == Base::ENGINE_MCRYPT) {
            $mcrypt = $rc2->encrypt('d');
            $this->assertEquals($result, $mcrypt,
                'Failed asserting that the mcrypt engine produced the correct result');
        } else {
        ...
        if ($rc2->getEngine() == Base::ENGINE_OPENSSL) {
            ...
            $this->assertEquals($result, $openssl,
                'Failed asserting that the OpenSSL engine produced the correct result');
        } else {
    ...
    public function testVectors($engine, $engineName, $key, $keyLen, $plaintext, $ciphertext)
    {
        ...
        $this->assertEquals($result, $ciphertext,
            "Failed asserting that $plaintext yielded expected output in $engineName engine");
    }
}
