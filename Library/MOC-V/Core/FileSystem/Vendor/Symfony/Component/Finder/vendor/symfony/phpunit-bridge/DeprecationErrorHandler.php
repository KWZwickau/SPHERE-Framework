...
class DeprecationErrorHandler
{
    ...
    public static function register( $mode = false )
    {
        ...
        $deprecationHandler = function ( $type, $msg, $file, $line, $context ) use ( &$deprecations, $mode ) {
            ...
            $i = count( $trace );
            while (isset( $trace[--$i]['class'] ) && ( 'ReflectionMethod' === $trace[$i]['class'] || 0 === strpos( $trace[$i]['class'],
                        'PHPUnit_' ) )) {
                // No-op
            }
            ...
            if (isset( $trace[$i]['object'] ) || isset( $trace[$i]['class'] )) {
                ...
                $group = 0 === strpos( $method, 'testLegacy' ) || 0 === strpos( $method,
                    'provideLegacy' ) || 0 === strpos( $method, 'getLegacy' ) || strpos( $class,
                    '\Legacy' ) || in_array( 'legacy', \PHPUnit_Util_Test::getGroups( $class, $method ),
                    true ) ? 'legacy' : 'remaining';
            ...
            register_shutdown_function( function () use ( $mode, &$deprecations, $deprecationHandler, $colorize ) {
                ...
                foreach (array( 'remaining', 'legacy', 'other' ) as $group) {
                    if ($deprecations[$group.'Count']) {
                        echo "\n", $colorize( sprintf( '%s deprecation notices (%d)', ucfirst( $group ),
                            $deprecations[$group.'Count'] ), 'legacy' !== $group ), "\n";
                        ...
                        foreach ($deprecations[$group] as $msg => $notices) {
                            ...
                            foreach ($notices as $method => $count) {
                                if ('count' !== $method) {
                                    echo '    ', $count, 'x in ', preg_replace( '/(.*)\\\\(.*?::.*?)$/', '$2 from $1',
                                        $method ), "\n";
                                }
                            }
                        }
