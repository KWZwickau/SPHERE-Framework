...
class Template extends Nette\Templating\FileTemplate
{
    ...
    public function __construct( Generator $generator )
    {
        ...
        $this->registerHelper( 'description', function ( $annotation, $context ) use ( $that ) {
            ...
            if ($context instanceof ReflectionParameter) {
                $description = preg_replace( '~^(\\$'.$context->getName().'(?:,\\.{3})?)(\s+|$)~i', '\\2', $description,
                    1 );
        ...
        $this->registerHelper( 'annotationFilter',
        function ( array $annotations, array $filter = array() ) use ( $todo, $internal ) {
        ...
        $this->registerHelper( 'sourceAnchors', function ( $source ) {
            ...
            $source = preg_replace_callback( '~(<span\\s+class="php-keyword1">(?:class|interface|trait)</span>\\s+)(\\w+)~i',
            function ( $matches ) {
            ...
            $source = preg_replace_callback( '~(<span\\s+class="php-keyword1">function</span>\\s+)(\\w+)~i',
            function ( $matches ) {
            ...
            // Constants
            $source = preg_replace_callback( '~(<span class="php-keyword1">const</span>)(.*?)(;)~is',
            function ( $matches ) {
            ...
            // Properties
            $source = preg_replace_callback( '~(<span\\s+class="php-keyword1">(?:private|protected|public|var|static)</span>\\s+)(<span\\s+class="php-var">.*?)(;)~is',
            function ( $matches ) {
    ...
    public function getClassUrl( $class )
    {
        ...
        return sprintf( $this->config->template['templates']['main']['class']['filename'],
    ...
    public function getConstantUrl( ReflectionConstant $constant )
    {
        ...
        return sprintf( $this->config->template['templates']['main']['constant']['filename'],
    ...
    public function getFunctionUrl( ReflectionFunction $function )
    {
        ...
        return sprintf( $this->config->template['templates']['main']['function']['filename'],
    ...
    private function resolveInternal( $text )
    {
        ...
        $internal = $this->config->internal;
        return preg_replace_callback( '~\\{@(\\w+)(?:(?:\\s+((?>(?R)|[^{}]+)*)\\})|\\})~',
        function ( $matches ) use ( $internal ) {
    ...
    public function getPackageUrl( $packageName )
    {
        ...
        return sprintf( $this->config->template['templates']['main']['package']['filename'],
    ...
    public function getNamespaceUrl( $namespaceName )
    {
        ...
        return sprintf( $this->config->template['templates']['main']['namespace']['filename'],
    ...
    public function getSourceUrl( ReflectionElement $element, $withLine = true )
    {
        ...
        $lines = null;
        if ($withLine) {
            $lines = $element->getStartLine() !== $element->getEndLine() ? sprintf( '%s-%s', $element->getStartLine(),
                $element->getEndLine() ) : $element->getStartLine();
        }

        return sprintf( $this->config->template['templates']['main']['source']['filename'],
            $file ).( null !== $lines ? '#'.$lines : '' );
    }
