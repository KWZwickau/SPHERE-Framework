    ...
    final public static function getValueDefinition(array $tokens, ReflectionElement $reflection)
    {
        ...
        if ($reflection instanceof ReflectionConstant || $reflection instanceof ReflectionFunction) {
            ...
            throw new Exception\RuntimeException('Invalid reflection object given.',
                Exception\RuntimeException::INVALID_ARGUMENT, $reflection);
        }
        ...
        if (!empty( $constants )) {
            foreach (array_reverse($constants, true) as $offset => $constant) {
                ...
                try {
                    switch ($constant) {
                        case '__LINE__':
                            throw new Exception\RuntimeException('__LINE__ constant cannot be resolved this way.',
                                Exception\RuntimeException::UNSUPPORTED, $reflection);
                        case '__FILE__':
                        ...
                        default:
                            if (0 === stripos($constant, 'self::') || 0 === stripos($constant, 'parent::')) {
                                ...
                                if ($reflection instanceof ReflectionConstant) {
                                    throw new Exception\RuntimeException('Constants cannot use self:: and parent:: references.',
                                        Exception\RuntimeException::UNSUPPORTED, $reflection);
                                ...
                                $constantName = self::resolveClassFQN($constant, $reflection->getNamespaceAliases(),
                                    $namespace);
