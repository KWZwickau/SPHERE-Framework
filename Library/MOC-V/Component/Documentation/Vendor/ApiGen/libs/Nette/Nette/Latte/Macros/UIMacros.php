...
class UIMacros extends MacroSet
{
    ...
    public static function install( Latte\Compiler $compiler )
    {
        ...
        $me->addMacro( 'ifCurrent', array( $me, 'macroIfCurrent' ),
            'endif' ); // deprecated; use n:class="$presenter->linkCurrent ? ..."
    ...
    public function finalize()
    {
        ...
        if ($this->namedBlocks) {
            foreach ($this->namedBlocks as $name => $code) {
                $func = '_lb'.substr( md5( $this->getCompiler()->getTemplateId().$name ), 0,
                        10 ).'_'.preg_replace( '#[^a-z0-9_]#i', '_', $name );
                $snippet = $name[0] === '_';
                $prolog[] = "//\n// block $name\n//\n"
    ...
    public function macroBlockEnd( MacroNode $node, PhpWriter $writer )
    {
        ...
        if (isset( $node->data->name )) { // block, snippet, define
            if ($node->name === 'snippet' && $node->htmlNode && !$node->prefix // n:snippet -> n:inner-snippet
                && preg_match( "#^.*? n:\w+>\n?#s", $node->content, $m1 ) && preg_match( "#[ \t]*<[^<]+$#sD",
                    $node->content, $m2 )
            ) {
