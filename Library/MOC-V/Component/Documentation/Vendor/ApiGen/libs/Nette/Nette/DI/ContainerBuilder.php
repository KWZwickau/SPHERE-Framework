...
use Nette;
use Nette\Utils\PhpGenerator\Helpers as PhpHelpers;
...
class ContainerBuilder extends Nette\Object
{
        ...
        foreach ($definitions as $name => $def) {
            try {
                ...
                foreach ($this->expand( $def->parameters ) as $k => $v) {
                    ...
                    $param = is_int( $k ) ? $method->addParameter( end( $tmp ) ) : $method->addParameter( end( $tmp ),
                        $v );
                    if (isset( $tmp[1] )) {
    ...
    private function resolveClass( $name, $recursive = array() )
    {
        ...
        if (isset( $recursive[$name] )) {
            throw new Nette\InvalidArgumentException( 'Circular reference detected for services: '.implode( ', ',
                    array_keys( $recursive ) ).'.' );
        }
        $recursive[$name] = true;
    ...
    public function getByType( $class )
    {
        ...
        if (!isset( $this->classes[$lower] )) {
            ...
            throw new ServiceCreationException( "Multiple services of type $class found: ".implode( ', ',
                    $this->classes[$lower] ) );
        }
    }
    ...
    public function formatStatement( Statement $statement, $self = null )
    {
        ...
        if (is_string( $entity ) && Strings::contains( $entity, '?' )) { // PHP literal
            ...
            foreach ($this->definitions[$service]->parameters as $k => $v) {
                $params[] = preg_replace( '#\w+$#', '\$$0',
                        ( is_int( $k ) ? $v : $k ) ).( is_int( $k ) ? '' : ' = '.PhpHelpers::dump( $v ) );
            }
