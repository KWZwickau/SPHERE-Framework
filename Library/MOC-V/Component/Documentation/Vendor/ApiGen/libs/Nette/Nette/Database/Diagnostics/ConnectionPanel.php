...
class ConnectionPanel extends Nette\Object implements Nette\Diagnostics\IBarPanel
{
    ...
    public function getPanel()
    {
        ...
        foreach ($this->queries as $i => $query) {
            ...
            $s .= '</td><td class="nette-DbConnectionPanel-sql">'.Helpers::dumpSql( self::$maxLength ? Nette\Utils\Strings::truncate( $sql,
                    self::$maxLength ) : $sql );
            if ($explain) {
            ...
            if ($source) {
                $s .= Nette\Diagnostics\Helpers::editorLink( $source[0],
                    $source[1] )->class( 'nette-DbConnectionPanel-source' );
            }
        ...
        return empty( $this->queries ) ? '' :
			...
			<h1>Queries: '.count( $this->queries ).( $this->totalTime ? ', time: '.sprintf( '%0.3f',
                    $this->totalTime * 1000 ).' ms' : '' ).'</h1>
