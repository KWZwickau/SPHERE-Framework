...
class Compiler extends Nette\Object
{
    ...
    public function writeMacro(
        ...
        if ($name[0] === '/') { // closing
            ...
            $isLeftmost = $node->content ? trim( substr( $this->output,
                    strrpos( "\n$this->output", "\n" ) ) ) === '' : false;
    ...
    public function expandMacro( $name, $args, $modifiers = null, HtmlNode $htmlNode = null, $prefix = null )
    {
        ...
        if (empty( $this->macros[$name] )) {
            $cdata = $this->htmlNodes && in_array( strtolower( end( $this->htmlNodes )->name ),
                    array( 'script', 'style' ) );
        ...
        foreach (array_reverse( $this->macros[$name] ) as $macro) {
            $node = new MacroNode( $macro, $name, $args, $modifiers,
                $this->macroNodes ? end( $this->macroNodes ) : null, $htmlNode, $prefix );
    ...
    private function processHtmlTagEnd( Token $token )
    {
        ...
        if (!$htmlNode->closing && ( strcasecmp( $htmlNode->name, 'script' ) === 0 || strcasecmp( $htmlNode->name,
                    'style' ) === 0 )
        ) {
