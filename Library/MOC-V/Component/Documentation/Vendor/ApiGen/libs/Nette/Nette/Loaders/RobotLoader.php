...
class RobotLoader extends AutoLoader
{
    ...
    public function tryLoad($type)
    {
        ...
        if (isset( $info[0] )) {
            ...
            if ($this->autoRebuild && !class_exists($type, false) && !interface_exists($type,
                    false) && ( PHP_VERSION_ID < 50400 || !trait_exists($type, false) )
            ) {
                $info = 0;
                $this->checked[$type] = true;
                if ($this->rebuilt) {
    ...
    private function scanDirectory($dir)
    {
        ...
        if (is_dir($dir)) {
            $ignoreDirs = is_array($this->ignoreDirs) ? $this->ignoreDirs : Strings::split($this->ignoreDirs,
                '#[,\s]+#');
            $disallow = array();
            ...
            $iterator = Nette\Utils\Finder::findFiles(is_array($this->acceptFiles) ? $this->acceptFiles : Strings::split($this->acceptFiles,
                '#[,\s]+#'))
                    ...
                    if (is_file("$path/netterobots.txt")) {
                        foreach (file("$path/netterobots.txt") as $s) {
                            if ($matches = Strings::match($s, '#^(?:disallow\\s*:)?\\s*(\\S+)#i')) {
                                $disallow[$path.str_replace('/', DIRECTORY_SEPARATOR,
                                    rtrim('/'.ltrim($matches[1], '/'), '/'))] = true;
                            }
                        }
                    }
