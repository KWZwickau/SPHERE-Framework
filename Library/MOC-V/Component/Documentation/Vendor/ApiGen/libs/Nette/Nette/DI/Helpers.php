...
final class Helpers
{
    ...
    public static function expand( $var, array $params, $recursive = false )
    {
        ...
        if (is_array( $var )) {
            ...
            return new Statement( self::expand( $var->entity, $params, $recursive ),
        ...
        foreach ($parts as $n => $part) {
            if ($n % 2 === 0) {
                ...
                throw new Nette\InvalidArgumentException( 'Circular reference detected for variables: '.implode( ', ',
                        array_keys( $recursive ) ).'.' );

            } else {
                $val = Nette\Utils\Arrays::get( $params, explode( '.', $part ) );
                if ($recursive) {
                    $val = self::expand( $val, $params,
                        ( is_array( $recursive ) ? $recursive : array() ) + array( $part => 1 ) );
                }
