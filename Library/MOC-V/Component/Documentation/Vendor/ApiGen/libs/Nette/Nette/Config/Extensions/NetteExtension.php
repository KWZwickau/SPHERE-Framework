...
class NetteExtension extends Nette\Config\CompilerExtension
{
    ...
    public function loadConfiguration()
    {
        ...
        if (isset( $config['session']['iAmUsingBadHost'] )) {
            $session->addSetup('Nette\Framework::$iAmUsingBadHost = ?;',
                array((bool)$config['session']['iAmUsingBadHost']));
        }
        ...
        foreach ((array)$config['database'] as $name => $info) {
            ...
            $connection = $container->addDefinition($this->prefix("database.$name"))
                ->setClass('Nette\Database\Connection',
                    array($info['dsn'], $info['user'], $info['password'], $info['options']))
            ...
            if ($info['reflection']) {
                $connection->addSetup('setDatabaseReflection', is_string($info['reflection'])
                    ? array(
                        new Nette\DI\Statement(preg_match('#^[a-z]+$#',
                            $info['reflection']) ? 'Nette\Database\Reflection\\'.ucfirst($info['reflection']).'Reflection' : $info['reflection'])
                    )
    ...
    public function afterCompile(Nette\Utils\PhpGenerator\ClassType $class)
    {
        ...
        foreach ((array)$config['forms']['messages'] as $name => $text) {
            $initialize->addBody('Nette\Forms\Rules::$defaultMessages[Nette\Forms\Form::?] = ?;',
                array($name, $text));
        }
