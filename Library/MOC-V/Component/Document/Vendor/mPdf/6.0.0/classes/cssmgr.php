                                    ...
                                    if (preg_match( '/^[.](.*)$/', $t, $m )) {
                                        ...
                                        if (preg_match( '/^[#](.*)$/', $t, $m )) {
                                            ...
                                            if (preg_match( '/^\[LANG=[\'\"]{0,1}([A-Z\-]{2,11})[\'\"]{0,1}\]$/', $t,
                                                ...
                                                if (preg_match( '/^:LANG\([\'\"]{0,1}([A-Z\-]{2,11})[\'\"]{0,1}\)$/',
                                                    $t, $m )) {
                                                    $tag = 'LANG>>'.strtolower( $m[1] );
                                                }    // mPDF 6  Special case for lang as attribute selector
                                                else {
...
//print_r($this->cascadeCSS); exit;
    ...
    function setCSStextshadow( $v )
    {
        ...
        foreach ($ss AS $s) {
            ...
            $p = explode( ' ', trim( $s ) );
            if (isset( $p[0] )) {
                $new['x'] = $this->mpdf->ConvertSize( trim( $p[0] ), $this->mpdf->FontSize, $this->mpdf->FontSize,
                    false );
            }
            if (isset( $p[1] )) {
                $new['y'] = $this->mpdf->ConvertSize( trim( $p[1] ), $this->mpdf->FontSize, $this->mpdf->FontSize,
                    false );
            }
            if (isset( $p[2] )) {
                if (preg_match( '/^\s*[\.\-0-9]/', $p[2] )) {
    ...
    function MergeCSS( $inherit, $tag, $attr )
    {
        ...
        if ($inherit == 'BLOCK') {
            ...
            //===============================================
            // Save Cascading CSS e.g. "div.topic p" at this block level
            $this->_mergeFullCSS( $this->cascadeCSS, $this->mpdf->blk[$this->mpdf->blklvl]['cascadeCSS'], $tag,
                $classes, $attr['ID'], $attr['LANG'] );
            //===============================================
        ...
        if ($inherit == 'BLOCK') {
            ...
            if ($inherit == 'INLINE') {
                ...
                if ($inherit == 'TOPTABLE' || $inherit == 'TABLE') { // NB looks at $this->tablecascadeCSS-1 for cascading CSS
                    ...
                    $this->_set_mergedCSS( $this->tablecascadeCSS[$this->tbCSSlvl - 1][$tag.'>>ID>>'.$attr['ID']], $p,
                        false, 9 );
                }
            }
        }
