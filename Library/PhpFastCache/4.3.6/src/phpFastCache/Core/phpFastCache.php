...
class phpFastCache
{
    ...
    public static $config = array(
        ...
        'overwrite' => "",
        // files, sqlite, etc it will overwrite ur storage and all other caching for waiting u fix ur server
        'allow_search' => false,
        // turn to true will allow $method search("/regex/")
        ...
        'extensions' => array(),
        "cache_method" => 2,
        // 1 = normal, 2 = phpfastcache, 3 = memory
        "limited_memory_each_object" => 128000,
    ...
    public static function getPath($skip_create_path = false, $config)
    {
        ...
        if ($skip_create_path == false && !isset(self::$tmp[$full_pathx])) {
            ...
            if (!@file_exists($full_path) || !@is_writable($full_path)) {
                ...
                if (!@file_exists($full_path) || !@is_writable($full_path)) {
                    throw new phpFastCacheCoreException('PLEASE CREATE OR CHMOD ' . $full_path . ' - 0777 OR ANY WRITABLE PERMISSION!',
                        92);
                }
            }
