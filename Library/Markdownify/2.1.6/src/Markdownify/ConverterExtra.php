...
class ConverterExtra extends Converter
{
    ...
    public function parseString( $html )
    {
        ...
        // => <fnref target="..." />
        $html = preg_replace( '@<sup id="fnref:([^"]+)">\s*<a href="#fn:\1" rel="footnote">\s*\d+\s*</a>\s*</sup>@Us',
            '<fnref target="$1" />', $html );
        // <div class="footnotes">
        // <hr />
        // <ol>
        ...
        //   ...
        // </footnotes>
        $html = preg_replace_callback( '#<div class="footnotes">\s*<hr />\s*<ol>\s*(.+)\s*</ol>\s*</div>#Us',
            array( &$this, '_makeFootnotes' ), $html );
    ...
    protected function handleTag_td()
    {
        ...
        if ($this->parser->isStartTag) {
            ...
            $this->table['col_widths'][$this->col] = max( $this->table['col_widths'][$this->col],
                $this->strlen( $buffer ) );
    ...
    protected function _makeFootnotes( $matches )
    {
        ...
        $fns = preg_replace( '@\s*(&#160;\s*)?<a href="#fnref:[^"]+" rev="footnote"[^>]*>&#8617;</a>\s*@s', '',
            $matches[1] );
