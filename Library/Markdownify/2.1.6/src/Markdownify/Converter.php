...
class Converter
{
    ...
    public function __construct(
        $linkPosition = self::LINK_AFTER_CONTENT,
    ...
    protected function parse()
    {
        ...
        $this->output = '';
        // drop tags
        $this->parser->html = preg_replace( '#<('.implode( '|', $this->drop ).')[^>]*>.*</\\1>#sU', '',
            $this->parser->html );
        while ($this->parser->nextNode()) {
            switch ($this->parser->nodeType) {
                ...
                case 'tag':
                    ...
                    if ($this->parser->isBlockElement && $this->parser->isNextToInlineContext && !in_array( $this->parent(),
                            $this->allowMixedChildren )
                    ) {
                        $this->setLineBreaks( 2 );
                    }
        ...
        // cleanup
        $this->output = rtrim( str_replace( '&amp;', '&',
                str_replace( '&lt;', '<', str_replace( '&gt;', '>', $this->output ) ) ) );
                ...
                if ($nowrap) {
                    ...
                    while ($pos = strpos( $put, "\n" )) {
                        ...
                        if ($lineLen + $putLen < $this->bodyWidth) {
                            ...
                            $split = preg_split( '#^(.{0,'.( $this->bodyWidth - $lineLen ).'})\b#', $putLine, 2,
                                PREG_SPLIT_OFFSET_CAPTURE | PREG_SPLIT_DELIM_CAPTURE );
                            $this->output .= rtrim( $split[1][0] )."\n".$this->indent.$this->wordwrap( ltrim( $split[2][0] ),
                                    $this->bodyWidth, "\n".$this->indent, false );
                        }
                    }
    ...
    protected function handleText()
    {
        ...
        if (!$this->hasParent( 'code' ) && !$this->hasParent( 'pre' )) {
            ...
            if (!$this->skipConversion) {
                ...
                $this->parser->node = preg_replace( $this->escapeInText['search'], $this->escapeInText['replace'],
                    $this->parser->node );
            }
        } else {
    ...
    protected function isMarkdownable()
    {
        ...
        if ($this->parser->isStartTag) {
            $return = true;
            if ($this->keepHTML) {
                $diff = array_diff( array_keys( $this->parser->tagAttributes ),
                    array_keys( $this->isMarkdownable[$this->parser->tagName] ) );
                if (!empty( $diff )) {
            ...
            if (!empty( $this->notConverted ) && end( $this->notConverted ) === $this->parser->tagName.'::'.implode( '/',
                    $this->parser->openTags )
            ) {
    ...
    protected function handleTag_ul()
    {
        ...
        if ($this->parser->isStartTag) {
            ...
            $this->unstack();
            if ($this->parent() != 'li' || preg_match( '#^\s*(</li\s*>\s*<li\s*>\s*)?<(p|blockquote)\s*>#sU',
                    $this->parser->html )
            ) {
