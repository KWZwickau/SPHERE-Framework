...
(function( b, c, g, h )
{
    ...
    h.Augment( {
        ...
        Startup: function()
        {
            ...
            d.GLYPH.defs = this.addElement( this.addElement( this.hiddenDiv.parentNode, "svg" ), "defs",
                {id: "MathJax_SVG_glyphs"} );
            this.ExSpan = g.Element( "span", {style: {position: "absolute", "font-size-adjust": "none"}},
                [["span", {className: "MathJax_SVG_ExBox"}]] );
            this.linebreakSpan = g.Element( "span", null,
        ...
        extendDelimiterV: function( p, x, j, m, l )
        {
            ...
            if (x > o) {
                ...
                while (n-- > 0) {
                    var q = h.Element( "g",
                        {transform: "translate(" + i.y + "," + (u - z * i.h + 50 + i.y) + ") scale(1," + z + ")"} );
        ...
        extendDelimiterH: function( q, m, j, o, l )
        {
            ...
            if (m > z) {
                ...
                while (p-- > 0) {
                    var r = h.Element( "g",
                        {transform: "translate(" + (v - i / 2 - B * u.l + u.x) + "," + u.y + ") scale(" + B + ",1)"} );
    ...
    var d = h.BBOX = MathJax.Object.Subclass( {
        type: "g", removeable: true, Init: function( i )
        {
                ...
                if (Math.abs( k.x ) < 1 && Math.abs( k.y ) < 1) {
                    ...
                    if (m === "g") {
                        if (!k.element.firstChild) {
                            ...
                            k.element.setAttribute( "transform",
                                "translate(" + Math.floor( k.x ) + "," + Math.floor( k.y ) + ")" )
                        }
                    } else {
    ...
    d.RECT = d.Subclass( {
        type: "rect", removeable: false, Init: function( j, l, i, k )
        {
            if (k == null) {
                k = {stroke: "none"}
            }
    ...
    d.FRAME = d.Subclass( {
        type: "rect", removeable: false, Init: function( l, o, i, k, n, j, m )
        {
            if (m == null) {
                m = {}
            }
    ...
    d.HLINE = d.Subclass( {
        type: "line", removeable: false, Init: function( j, l, p, k, o )
        {
            if (o == null) {
            ...
            if (p === "dashed") {
                var q = Math.floor( Math.max( 0, j - l ) / (6 * l) ), i = Math.floor( Math.max( 0,
                    j - l ) / (2 * q + 1) );
    ...
    d.VLINE = d.Subclass( {
        type: "line", removeable: false, Init: function( l, k, p, j, o )
        {
            if (o == null) {
                o = {"stroke-linecap": "square"}
            }
            ...
            if (p === "dashed") {
                var q = Math.floor( Math.max( 0, l - k ) / (6 * k) ), i = Math.floor( Math.max( 0,
                    l - k ) / (2 * q + 1) );
                o["stroke-dasharray"] = i + " " + i
            }
            if (p === "dotted") {
    ...
    d.TEXT = d.Subclass( {
        type: "text", removeable: false, Init: function( l, k, i )
        {
            if (!i) {
                i = {}
            }
    ...
    d.GLYPH = d.Subclass( {
        type: "path", removeable: false, Init: function( q, k, v, x, y, s, j, m )
        {
    ...
    c.Register.StartupHook( "mml Jax Ready", function()
    {
        ...
        a.mbase.Augment( {
            ...
            }, SVGhandleColor: function( m )
            {
                ...
                if (v.mathcolor) {
                    h.Element( m.element, {fill: v.mathcolor, stroke: v.mathcolor} );
                    m.removeable = false
                }
                ...
                if (v.background !== a.COLOR.TRANSPARENT) {
                    ...
                    m.Add( d.RECT( m.h, m.d, m.w, {fill: v.background, stroke: "none"} ), 0, 0, false, true )
                }
                if (q) {
                    var u = 5;
                    var k = {
        ...
        a.mroot.Augment( {
            toSVG: a.msqrt.prototype.toSVG, SVGaddRoot: function( l, j, o, m, i )
            {
        ...
        a.munderover.Augment( {
            toSVG: function( E, B )
            {
                ...
                var n = o[this.base] || {w: 0, h: 0, d: 0, H: 0, D: 0, l: 0, r: 0, y: 0, scale: L};
                ...
                for (I = 0, F = this.data.length; I < F; I++) {
                    if (this.data[I] != null) {
                        ...
                        z = {left: 0, center: (H - s.w) / 2, right: H - s.w}[j.align];
                        r = z;
                        p = 0;
                        if (I == this.over) {
                            if (w) {
        ...
        a.math.Augment( {
            SVG: d.Subclass( {type: "svg", removeable: false} ), toSVG: function( t, j )
            {
                var v = h.config;
                if (this.data[0]) {
                    this.SVGgetStyles();
                    ...
                    q.element.setAttribute( "viewBox",
                        h.Fixed( -o, 1 ) + " " + h.Fixed( -q.H - h.em, 1 ) + " " + h.Fixed( o + q.w + i,
                            1 ) + " " + h.Fixed( q.H + q.D + 2 * h.em, 1 ) );
                    ...
                    if (!this.isMultiline && this.Get( "display" ) === "block" && !q.hasIndent) {
                        ...
                        if (m) {
                            c.Insert( k, ({
                                ...
                                center: {marginLeft: h.Ex( m ), marginRight: h.Ex( -m )}
                            })[u.indentalign] )
                        }
                    }
                }
